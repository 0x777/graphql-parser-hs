module Language.GraphQL.Request
  ( GQLReq(..)
  , GQLReqUnparsed
  , GQLReqParsed
  , toParsed
...
data GQLReq a
  = GQLReq
  { _grOperationName :: !(Maybe G.OperationName)
  , _grQuery         :: !a
...
toParsed :: (MonadError Text m ) => GQLReqUnparsed -> m GQLReqParsed
toParsed req = case G.parseExecutableDoc gqlText of
  Left _  -> throwError "not a valid graphql query"
