module Language.GraphQL.Validate.Types
  ( InpValInfo(..)
  , ParamMap
...
import           Language.GraphQL.Internal.Instances
...
type EnumValuesInfo = (Map.HashMap G.EnumValue EnumValInfo)
...
instance EquatableGType EnumTyInfo where
  type EqProps EnumTyInfo = (G.NamedType, EnumValuesInfo)
...
fromEnumTyDef (G.EnumTypeDefinition descM n _ valDefs) =
  EnumTyInfo descM (G.NamedType n) enumVals
  where
...
mkHsraObjFldInfo descM name params ty =
  ObjFldInfo descM name params ty
...
mkObjTyInfo
  ...
  -> IFacesSet -> ObjFieldMap -> ObjTyInfo
mkObjTyInfo descM ty iFaces flds =
...
mkHsraObjTyInfo descM ty implIFaces flds =
  mkObjTyInfo descM ty implIFaces flds

mkIFaceTyInfo
  :: Maybe G.Description -> G.NamedType
  -> Map.HashMap G.Name ObjFldInfo -> IFaceTyInfo
...
typenameFld =
  ObjFldInfo (Just desc) "__typename" Map.empty
    (G.toGraphQLType $ G.toNonNullType $ G.NamedType "String")
  where
    desc = "The name of the current Object type at runtime"

fromObjTyDef :: G.ObjectTypeDefinition -> ObjTyInfo
fromObjTyDef (G.ObjectTypeDefinition descM n ifaces _ flds) =
  mkObjTyInfo descM (G.NamedType n) (Set.fromList ifaces) fldMap
  where
    fldMap =
...
fromIFaceDef :: G.InterfaceTypeDefinition -> IFaceTyInfo
fromIFaceDef (G.InterfaceTypeDefinition descM n _ flds) =
  mkIFaceTyInfo descM (G.NamedType n) fldMap
  where
    fldMap =  Map.fromList [(G._fldName fld, fromFldDef fld) | fld <- flds]
...
fromInpObjTyDef (G.InputObjectTypeDefinition descM n _ inpFlds) =
  InpObjTyInfo descM (G.NamedType n) fldMap
  where
    fldMap = Map.fromList
...
mkHsraInpTyInfo descM ty flds =
  InpObjTyInfo descM ty flds

data ScalarTyInfo
  = ScalarTyInfo
  { _stiDesc :: !(Maybe G.Description)
  , _stiType :: !G.NamedType
  } deriving (Show, Eq, TH.Lift)

-- mkHsraScalarTyInfo :: ScalarType -> ScalarTyInfo
-- mkHsraScalarTyInfo ty = ScalarTyInfo Nothing ty
...
fromScalarTyDef (G.ScalarTypeDefinition descM n _) =
  return $ ScalarTyInfo descM ty
  where
    ty = G.NamedType n
    -- ty = case n of
    --   "Int"     -> return ScalarInt
    --   "Float"   -> return ScalarFloat
    --   "String"  -> return ScalarString
    --   "Boolean" -> return ScalarBoolean
    --   _         -> return $ ScalarCustom $ G.NamedType n
...
data ScalarType
  ...
  | ScalarFloat
  | ScalarBoolean
  | ScalarCustom !G.NamedType
  deriving(Show, Eq, TH.Lift)

-- map postgres types to builtin scalars
pgColTyToScalar :: ScalarType -> Text
pgColTyToScalar = \case
  ScalarInt      -> "Int"
  ScalarBoolean  -> "Boolean"
  ScalarFloat    -> "Float"
  ScalarString   -> "String"
  ScalarCustom t -> G.unName $ G.unNamedType t
...
defaultDirectives =
  ...
  where
    ...
    args = Map.singleton "if" $ InpValInfo Nothing "if" Nothing $
           G.toGraphQLType $ G.toNonNullType $ mkScalarTy ScalarBoolean
...
data AnnGValue
  = AGScalar !ScalarType !(Maybe ScalarValue)
  | AGEnum !G.NamedType !(Maybe G.EnumValue)
  | AGObject !G.NamedType !(Maybe AnnGObject)
  | AGArray !(G.ListType G.NamedType) !(Maybe [AnnInpVal])
  deriving (Show, Eq)
